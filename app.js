...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateStatusDisplay() {
        ...
        // 更新最后更新时间
        const now = new Date();
        const diff = Math.floor((now - statusData.lastUpdate) / 1000);
        
        if (diff < 60) {
            lastUpdateEl.textContent = '刚刚';
        } else if (diff < 3600) {
            lastUpdateEl.textContent = `${Math.floor(diff / 60)}分钟前`;
        } else {
            lastUpdateEl.textContent = `${Math.floor(diff / 3600)}小时前`;
        }
    ...
    document.addEventListener('keydown', function(e) {
        // ESC 关闭模态框
        if (e.key === 'Escape' && modelModal.style.display === 'flex') {
            closeModelModal();
        }
